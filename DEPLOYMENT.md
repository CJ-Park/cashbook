...
2026-07-14에 Vercel GitHub 앱의 private 저장소 권한을 부여하고 `CJ-Park/cashbook`을 프로젝트에 연결했다. 작업 브랜치 push는 Preview, `main` push는 Production 자동 배포를 생성한다. DB migration이 포함된 변경은 운영 DB 적용·검증을 마친 뒤에만 `main`에 반영한다.
...
- Vercel Git 자동 배포가 실패하거나 고위험 변경의 후보 승격이 필요하면 CLI 후보 배포·검증·승격 절차를 사용한다.
