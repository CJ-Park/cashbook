...
Vercel GitHub 앱의 저장소 접근 권한은 아직 연결되지 않았다. 2026-07-14에 `CJ-Park/cashbook` 연결을 다시 시도했지만 private 저장소 접근 권한 부족으로 실패했다. 저장소 소유자가 Vercel GitHub 앱에 권한을 부여하기 전까지는 아래 CLI 명령으로 Production을 배포한다.

## 현재 변경분의 운영 반영 상태

2026-07-14에 DB TLS·데이터 무결성·20건 무한 스크롤·Excel 1년 기간 제한·데스크톱 카테고리 수정 폼 변경을 Production에 반영했다. 운영 DB, Preview, Production 검증 결과는 다음과 같다.
...
- Preview에서 임시 인증 사용자의 로그인, 기본 카테고리 16개, 거래 CRUD, 20·20·5건 무한 스크롤, 필터, Excel 1년 경계, PWA metadata, 세션 만료, 로그아웃 E2E 통과
- Production 후보 `dpl_8Vvz7XVxMA4e9NAM4omL6qDEzdGu`를 `--skip-domain`으로 생성하고 동일 E2E와 1440px 데스크톱 카테고리 수정 폼 overflow·입력 너비 검증 통과
- 후보 검증 중 Vercel error 로그와 HTTP 500 응답 0건 확인 후 Production alias 승격
- 실제 Production URL에서 동일 인증 E2E 재통과, 임시 거래·카테고리·profile·Auth 사용자 잔여 0건 확인
- Production HTTP smoke에서 로그인 `200`, 보호 화면 `307`, 거래 API `401`, Excel API `307`, manifest·아이콘 `200`, 서울 리전 `icn1` 확인
- CSP, HSTS, `X-Frame-Options: DENY`, `nosniff`, Referrer·Permissions Policy 확인과 `Server-Timing: auth`·`X-Cashbook-Auth-Duration` 부재 확인
- 최종 Vercel error 로그와 HTTP 500 응답 0건, 점검용 Vercel 자동화 우회 토큰 잔여 0개 확인

자동화할 수 있는 현재 릴리스의 운영 검증은 완료했다. 외부 기기·권한이 필요한 잔여 항목은 다음과 같다.

- Android Chrome·iPhone/iPad 실기기 PWA 설치·세션 확인
- Vercel GitHub 앱의 private 저장소 접근 권한 부여 후 `main` 자동 배포 연결
...
npm test
npm run build
npx vercel@latest deploy --prod --skip-domain --yes --scope joe-private
npx vercel@latest promote <검증된-candidate-url> --yes --scope joe-private
```
...
- Android Chrome에서 앱 설치 후 아이콘·이름·standalone 실행 확인
- iPhone/iPad에서 `공유` → `홈 화면에 추가` → `웹 앱으로 열기(Open as Web App)`를 켠 뒤 아이콘·이름·standalone 실행 확인
...
- 당시 Production에서 거래 목록·검색 결과 합계를 단일 SQL 요청으로 확인했다. 현재 Production은 DB 전체 합계와 20건 커서 페이지를 병렬 조회한다.
...
## 2026-07-14 운영 하드닝 배포 검증

- `main` commit `a952b05`와 Production deployment `dpl_8Vvz7XVxMA4e9NAM4omL6qDEzdGu` 반영
- Preview·Production 후보·실제 Production에서 로그인 세션 기반 런타임 DB 연결과 전체 E2E 통과
- 거래 CRUD, 최초 20건과 후속 20·5건 무한 스크롤, 필터별 전체 합계, 잘못된 커서·날짜 차단 확인
- Excel은 양쪽 날짜 필수, 최대 1년 허용, 초과 기간 차단과 45건 workbook 내용을 확인
- 데스크톱 출금·입금 카테고리 수정 폼을 세로 배치해 이름 입력칸 축소와 overflow를 해소하고 1440·1024·390px에서 검증
- Chrome manifest 차단 오류 0건, 일반 192·512·maskable 512·Apple 180 아이콘의 응답·MIME·실제 픽셀 크기 확인
- 서비스 워커 등록 0개와 Cache Storage key 0개 확인
- 세션 만료 redirect와 로그아웃 확인
- E2E 임시 사용자 데이터와 자동화 우회 토큰 잔여 0 확인
- 운영 error 로그와 HTTP 500 응답 0건, 인증시간 응답 헤더 부재 확인

## 잔여 운영 항목

- Vercel GitHub 앱에 `CJ-Park/cashbook` private 저장소 접근 권한을 부여한 뒤 Git 자동 배포를 연결한다. 그전까지 Production은 검증 후 CLI로 배포한다.
- Android Chrome과 iPhone/iPad 실기기에서 설치·standalone·safe area·설치 앱 내부 로그인/세션 만료/로그아웃을 확인한다.
