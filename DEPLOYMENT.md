...
배포 후 응답의 `X-Vercel-Id`에 함수 실행 리전이 `icn1`로 표시되는지 확인한다. Proxy 인증 시간은 로컬에서 `Server-Timing: auth;dur=...`, Vercel Production에서 `X-Cashbook-Auth-Duration: ...ms` 응답 헤더로 확인한다. Vercel은 Proxy에서 설정한 `Server-Timing`을 외부 응답에서 제거할 수 있어 같은 비민감 측정값을 커스텀 헤더로 함께 제공한다.
